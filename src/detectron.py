...
def detect_video(predictor, video_path, class_mapper):
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            ob_names = np.array(ob_names)
            mask = []

            for i in range(len(ob_names)):
                if ob_names[i] == 'person' or ob_names[i] == 'dog':
                    mask.append(True)
                else:
                    mask.append(False)
            ...
            ob_names = ob_names[mask]

            #output: xyxy
            outputs, class_names = deepsort.update(bbox_xcycwh, np.array(cls_conf), ob_names, frame)
            if len(outputs) > 0:
                track_ids = outputs[:, 4]
                bbox_xyxy = outputs[:, :4]
                bbox_xcycwh = []
            ...
            if frame_id > 2:
                ...
                for j in bbox_xcycwh:
                    ...
                    score = 0 #float(cls_conf[i])
                    h = int(j[3])
                    w = int(j[2])
                    object_info = {'cam_id': CAM, 'id': int(track_ids[i]), 'mask_com': commask, 'ground_point': ground_point, 'class': object_name, 'score': score, 'height': h, 'width': w}
                    info2[frame_id].append(object_info)
                    i+=1
                    if len(bbox_xcycwh) > len(class_names):
                        #print(len(ob_names))
            ...
            print(f"Cam: {CAM} || Frame no: {frame_id} || Current FPS: {round(fps,2)}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    for filename in filenames:
        CAM = int(re.search(r'\d+', filename).group(0))
        video_path = os.path.join(os.path.join(VIDEO_DIR, TEST_NAME), filename)
        name = filename.split('.')[0] + ".json"
        print(os.path.join(OUTPUT_DIR, name))
        info = detect_video(predictor, video_path, class_mapper)
