...
class YOLOv7_DeepSORT:
    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        ...
        frame_num = 0
        info = {}
        pbar = tqdm(total=vid.get(cv2.CAP_PROP_FRAME_COUNT))
        while True: # while video is running
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                cv2.circle(frame, (int((bbox[0] + bbox[2]) / 2), int((bbox[1] + bbox[3]) / 2 + int(bbox[3] - bbox[1])/2)), 5, (255, 0, 0), -1)
                ...
                object_info = {'cam_id': CAM, 'id': int(track.track_id), 'mask_com': commask, 'ground_point': ground_point, 'class': object_name, 'score': score, 'height': h, 'width': w}
                info[frame_num].append(object_info)

            # -------------------------------- Tracker work ENDS here -----------------------------------------------------------------------
            if verbose >= 1:
                fps = 1.0 / (time.time() - start_time) # calculate frames per second of running detections
                if not count_objects: pbar.set_description(f"Processed frame no: {frame_num} || Current FPS: {round(fps,2)}")
                else: pbar.set_description(f"Processed frame no: {frame_num} || Current FPS: {round(fps,2)} || Objects tracked: {count}")
...
if __name__ == "__main__":
    detector = Detector(classes = [16]) # humans 0, dogs 16
    ...
    for filename in filenames:
        ...
        print(f"video: {video_num}/5: {output_path}")
        info = tracker.track_video(video_path, output=None, show_live = True, skip_frames = 0, count_objects = True, verbose=1)
