    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            if len(outputs) > 0:
                ...
                bbox_xcycwh = np.array(bbox_xcycwh, dtype=np.int)
                #print(f"updated: {bbox_xcycwh}")

            print(f"ori {frame_id}: {info[frame_id]}")
            if frame_id > 2:
                i = 0
                for j in bbox_xcycwh:
                    ...
                    h = j[3]
                    w = j[2]
                    object_info = {'cam_id': CAM, 'id': str(track.track_id), 'mask_com': commask, 'ground_point': ground_point, 'class': object_name, 'score': score, 'height': h, 'width': w}
            ...
            print(f"tracked {frame_id}: {info2[frame_id]}")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(OUTPUT_DIR, TEST_NAME + "_det.json"), 'w') as f:
